---
trigger: always_on
description: The 12-step engineering methodology for implementing changes in Vermeil, from intent clarification to validation and commits.
---
...
- Dependency and environment footprint: if the change adds, removes, or version-bumps a dependency or a required tool (Rust crate, npm package, Gradle/Java mod dep, JDK, Build Tools, system lib), the manifests **and** the docs that list prerequisites must move with it — `docs/DEVELOPMENT.md` Prerequisites and the relevant skill. A stale "what to install" list is a bug. See the `dependencies` skill (`.agents/skills/dependencies/SKILL.md`).
...
What this step does NOT cover — these belong to the `release-process` skill (`.agents/skills/release-process/SKILL.md`) and only happen when the user explicitly says "release":
...
- **Dependency & toolchain manifests + their docs** — a new/removed/bumped dep or
  ...
  `dependencies` skill (`.agents/skills/dependencies/SKILL.md`).
