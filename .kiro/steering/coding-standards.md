...
naming conventions, **see the `minecraft-mod` skill** — these rules don't repeat
it here.
...
3. **Backend Resolution (`modpack.rs`, `cf_import.rs`, `icon_cache.rs`)**: Resolve icons through the hierarchy (embedded archive icon → API SHA-1 lookup → API title search → `"cube"` fallback). Set `LoaderConfig`, `source_project_id`, `source_platforms`, and `source_version` (pinning is strictly user-driven via manual pin controls; newly created instances are not auto-pinned).
