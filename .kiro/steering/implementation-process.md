...
- **Linux output-capture bug.** On Linux, Kiro's `execute_bash` currently swallows stdout/stderr (returns exit 0 with empty output) due to a known shell-integration/OSC-marker bug (kirodotdev/Kiro issues #8744, #4833). The workaround is to redirect a command's output to a workspace file and read it back — but **only do this when the agent actually needs to see the output to decide what to do next.** Examples of when it's needed: probing system state (versions, presence of tools, search results), build/test pass-fail messages, anything whose content drives the next step. Examples of when it's **not** needed and the workaround is pure waste: `rm`, `mv`, `chmod`, `git add`, `git commit`, `git push`, `cargo check` / `pnpm build` invocations whose exit code is the only thing that matters. For those, run the command directly and trust the exit code; verify outcomes with a follow-up read (e.g. `git status` after commits) only if needed. When output IS needed, batch related commands into one shell call writing one combined file, not separate files per command. Clean the file up the same call you read it. A fresh Kiro reload with shell integration disabled may restore direct capture, in which case skip the workaround entirely.
