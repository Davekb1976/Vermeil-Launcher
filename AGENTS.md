...
3. **Backend Resolution (`modpack.rs`, `cf_import.rs`, `icon_cache.rs`, `instance_service.rs`)**: Resolve icons through the hierarchy (embedded archive icon → API SHA-1 lookup → API title search → `"cube"` fallback). **Invariant**: Always persist the resolved icon into the instance's own directory (`<instance_dir>/icon.<ext>`) via `icon_cache::persist_instance_icon()`, never pointing `instance.icon` to volatile `%LOCALAPPDATA%\Vermeil\cache\` (wiped on purge cache). In `instance_service::sanitize_instance_json()`, automatically heal missing icon paths to `"cube"` or migrate legacy cache paths to `<instance_dir>/icon.<ext>`. Set `LoaderConfig`, `source_project_id`, `source_platforms`, and `source_version` (pinning is strictly user-driven via manual pin controls; newly created instances are not auto-pinned). Trigger `platform::update_windows_estimated_size()` to synchronize the Windows uninstaller storage footprint.
