  ...
  return (
    <div class="screen-enter">
      ...
      {/* Floating action bar — appears at bottom-center when in select mode.
          Shows selected count + Delete; clicking Delete reveals the confirm input. */}
      <Show when={selectMode()}>
        <div class="library-delete-bar">
          ...
          <Show when={!showDeleteConfirm()} fallback={
            <>
              <input class="field-control field-control--text" style="max-width:120px;border-color:var(--danger)" placeholder="Type Confirm" value={deleteInput()} onInput={(e) => setDeleteInput(e.currentTarget.value)} />
            ...
            <button class="btn btn--danger" disabled={selected().size === 0} onClick={async () => {
              ...
              if (settings.force_delete) { await deleteSelected(); } else { setShowDeleteConfirm(true); }
            }}>Delete ({selected().size})</button>
            <button class="btn btn--ghost" onClick={() => { setSelectMode(false); setSelected(new Set<string>()); }}>Cancel</button>
          </Show>
        </div>
      </Show>
