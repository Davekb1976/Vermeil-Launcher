 ...
 * sidebar, search bar, setting rows with an ON/OFF pill) in the launcher's
 * dark/purple palette using the vanilla pixel font.
 */
public class VermeilSettingsScreen extends Screen {
	...
	public boolean keyPressed(final KeyEvent event) {
		...
		if (searchFocused && keyCode == KEY_BACKSPACE) {
			if (!search.isEmpty()) {
	...
	//? if <=1.21.11 {
	/*@Override
	public void render(final GuiGraphics gfx, final int mouseX, final int mouseY, final float delta) {
		gfx.fill(x0, y0, x1, y1, PANEL);
		gfx.fill(x0, y0, sidebarRight, y1, SIDEBAR);
		gfx.fill(sidebarRight, y0, sidebarRight + 1, y1, BORDER);
		outline(gfx, x0, y0, x1, y1, BORDER);
	...
	private void drawSearch(final GuiGraphics gfx) {
		gfx.fill(contentX, searchY, contentRight, searchY + searchH, FIELD);
		outline(gfx, contentX, searchY, contentRight, searchY + searchH, searchFocused ? ACCENT : BORDER);
		boolean showPlaceholder = search.isEmpty() && !searchFocused;
		String shown = showPlaceholder ? "Search modules..." : search;
	...
	private void outline(final GuiGraphics gfx, final int ax, final int ay, final int bx, final int by, final int color) {
		gfx.fill(ax, ay, bx, ay + 1, color);
		gfx.fill(ax, by - 1, bx, by, color);
		gfx.fill(ax, ay, ax + 1, by, color);
		gfx.fill(bx - 1, ay, bx, by, color);
	...
	private void scaledText(final GuiGraphics gfx, final String s, final int x, final int y, final int color, final float scale, final boolean shadow) {
		if (scale == 1.0F) {
			gfx.drawString(this.font, s, x, y, color, shadow);
			return;
		}
		gfx.pose().pushMatrix();
		gfx.pose().scale(scale, scale);
		gfx.drawString(this.font, s, Math.round(x / scale), Math.round(y / scale), color, shadow);
		gfx.pose().popMatrix();
	}
	*///?}
