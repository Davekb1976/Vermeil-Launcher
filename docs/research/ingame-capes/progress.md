...
## Forge 1.8.9 companion mod (coremod cape hook)
- New `companion-mod/forge/1.8.9/`: the legacy PvP era. Chosen over Legacy Fabric — that audience runs Forge for the Sodium/OptiFine performance-mod ecosystem Legacy Fabric lacks. Single version (no range).
- Toolchain (verified by bootstrapping): classic ForgeGradle `2.1-SNAPSHOT` from `maven.minecraftforge.net` + Gradle 3.1 wrapper + JDK 8 (portable, pinned via `org.gradle.java.home`) + Forge `1.8.9-11.15.1.2318-1.8.9` + MCP `stable_22`. `:decompileMc` OOMs at 1G → `-Xmx3G`.
- Hook = **coremod, not Mixin** (1.8.9 predates our Mixin toolchain). `VermeilLoadingPlugin` (IFMLLoadingPlugin, SortingIndex 1001) registers `VermeilCapeTransformer` (IClassTransformer) which injects at the head of `AbstractClientPlayer.getLocationCape()`: `if (overrideCapeLocation(this) != null) return it;`. Verified target from genSources — MCP `getLocationCape` / SRG `func_110303_q`, desc `()Lnet/minecraft/util/ResourceLocation;`. Dev-vs-prod name picked via `fml.deobfuscatedEnvironment`. ASM: EXPAND_FRAMES read + COMPUTE_MAXS write + one hand-written branch frame (avoids whole-class recompute / classloading mid-transform).
- `VermeilCape` (client): polls `<dataDir>/cape.{png,json}` ~1s on `ClientTickEvent`, decodes via `ImageIO`→`BufferedImage`, crops each square frame's top 2:1 panel (W×W/2), registers a `DynamicTexture` under `vermeil:cape`, animates by re-uploading frame int[] each tick. Same launcher contract (`vermeil.dataDir`, square-slot strip) as the Fabric mods.
