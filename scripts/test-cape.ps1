...
param(
  ...
  #   dev      = companion-mod\stonecutter\run\vermeil (or companion-mod\forge\1.8.9\run\vermeil)
  [ValidateSet('launcher', 'dev')]
  [string]$Target = 'launcher',
  # Which mod project's dev client to target when -Target dev:
  #   stonecutter (modern Fabric & NeoForge: 1.21.11, 26.1, 26.2, 26.3)
  #   forge       (legacy Forge 1.8.9)
  [ValidateSet('stonecutter', 'forge')]
  [string]$DevProject = 'stonecutter'
)
...
if ($Target -eq 'dev') {
  ...
  if ($DevProject -eq 'forge') {
    $companion = Join-Path $repo "companion-mod\forge\1.8.9\run\vermeil"
  } else {
    $companion = Join-Path $repo "companion-mod\stonecutter\run\vermeil"
  }
} else {
